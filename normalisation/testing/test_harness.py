...
# === CONFIG ===
NORMALISE_SCRIPT = "***REMOVED***/mono_normalise.py"
...
# def run_normaliser(lang, infile, outfile):
# 	cmd = [
...
def run_normaliser(lang, infile):
	...
	try:
		...
		# Extract the normaliser script path from stdout
		norm_file = None
...
def check_idempotence(lang, outfile):
	# make a temp dir for the second run
	IDEM_OUT = pathlib.Path("idem_output")
	...
	# in the temp dir, the normaliser will write {lang}_norm.txt
	generated = IDEM_OUT / f"{lang}_norm.txt"
	if not generated.exists():
		return False

	# compare original and idem
...
def analyse(lang, infile, outfile):
	...
	ratios = []
	for a, b in zip(inp_lines, out_lines):
		if len(a) > 0:
			ratios.append(len(b) / len(a))
	...
	if changed_pct == 0:
		notes.append("No changes (maybe normaliser missing?)")
	if changed_pct == 100:
...
# 		ok, err = run_normaliser(lang, infile, outfile)
# 		if not ok:
# 			results.append({
# 				"lang": lang,
# 				"lines_in": 0,
...
# 			"mean_len_ratio", "idempotent", "notes"
# 		])
# 		writer.writeheader()
