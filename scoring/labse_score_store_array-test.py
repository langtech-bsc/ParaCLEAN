...
from languages import language_identification, idioma_languages, fast_text_languages
...
filter_langs = [Language.CATALAN, Language.SPANISH]
...
def add_base_arguments_to_parser(parser):
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to the split input file to score'
    )
    parser.add_argument(
        '--output',
        type=str,
...
def detect_with_fasttext(sentence, language):
    predictions=fasttext_detector.predict(sentence, k=5)
    predicted_languages = predictions[0]
    print(predicted_languages, flush=True) 
    probabilities = predictions[1]
    print(probabilities, flush=True)

    fasttext_code = fast_text_languages[language]
    print("target language is: ", fasttext_code, flush=True)

    target_language_code = f"__label__{fasttext_code}"
...
if __name__ == "__main__":
    ...
    model=SentenceTransformer('***REMOVED***/labse_model')
    print('model loaded')

    idioma_detector = joblib.load("***REMOVED***/idioma_cognitor/model.pkl")
    ...
    fasttext_detector = fasttext.load_model("***REMOVED***/fast_text/model.bin")
    filter_detector = LanguageDetectorBuilder.from_languages(*filter_langs).build()
    ...
    with open(output_path, 'w') as output:
        ...
        if l1 in idioma_default:
            ...
            print(l1 + " is a fasttext language")
        ...
        if l2 in idioma_default:
            ...
            print(l2 + " is a fasttext language")
